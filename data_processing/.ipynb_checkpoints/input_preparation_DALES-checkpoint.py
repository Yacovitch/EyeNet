...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset_path', type=str, default='/nas2/jacob/DALES/dales_ply_to_ply', help='')
    parser.add_argument('--output_path', type=str, default='/nas2/jacob/DALES', help='original dataset path')
    parser.add_argument('--grid_size', type=float, default=0.250, help='sampling grid size default=0.2')
    ...
    for sample_type in preparation_types:
        for pc_path in files:
            ...
            xyz, labels = DP.read_ply_data(pc_path, with_rgb=False)
            ...
            #if sample_type == 'grid':
            sub_xyz, sub_labels = DP.grid_sub_sampling(xyz, None, labels, grid_size)
            #elif sample_type == 'raw':
            #    print('processing raw files')
            #    sub_xyz, sub_i, sub_labels = xyz, i, labels
            #else:
            #    sub_xyz, sub_i, sub_labels = DP.random_sub_sampling(xyz, i, labels, random_sample_ratio)

            #sub_rgb = sub_rgb / 255.0
            sub_labels = np.squeeze(sub_labels)
            #write_ply(sub_ply_file, [sub_xyz, sub_i, sub_labels], ['x', 'y', 'z', 'num_return', 'class'])
