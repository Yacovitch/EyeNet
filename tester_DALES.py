                ...
                if last_min + 1 < new_min:
                    ...
                    if int(np.ceil(new_min)) % 1 == 0:
                        ...
                        for i_test in range(num_test):
                            ...
                            pc_path = dataset.path + '/dales_ply_to_ply/test/' + name[:-4] + '.ply'
                            print(pc_path)
                            xyz, _ = DP.read_ply_data(pc_path, with_rgb = False, with_label = True, with_i = False)
                            write_ply(join(test_path, 'test_preds', name), [xyz, preds, labels], ['x','y','z','pred', 'label'])
